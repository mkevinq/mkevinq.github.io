...
const HobbyTile: FunctionComponent<Hobby> = ({ title, content, image }) => {
  ...
  return (
    <div className="mx-auto w-5/6 md:w-5/6 m-2 cursor-pointer">
      <div
        className="text-white flex items-center rounded-xl drop-shadow-md"
        onClick={() => {
          setContentVisibility(!isContentVisible);
        }}
      >
        <h2 className="m-4 md:m-8 absolute md:text-4xl font-bold drop-shadow-md">{title}</h2>
        {isContentVisible ? (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="z-10 right-0 absolute m-4 md:m-8 text-white h-6 md:h-12 w-6 md:w-12 drop-shadow-sm"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <video className="h-16 md:h-32 w-full object-cover rounded-xl" autoPlay loop muted>
          <source src={image} />
        </video>
      </div>
      <div
